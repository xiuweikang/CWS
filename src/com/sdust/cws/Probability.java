...
public class Probability {
	 ...
	 * */
	public void createWordResult(Map<String, Map<String, Double>> result,
			Map<String, List<String>> countryDic,
			Map<String, List<Integer>> commenWord) {
		for (Iterator<String> it = commenWord.keySet().iterator(); it.hasNext();) {
			String word = it.next();
			Map<String, Double> temp = new HashMap<String, Double>();
			for (Iterator<String> is = countryDic.keySet().iterator(); is
					.hasNext();) {
	 ...
	 * 构造存放每个非国家单词，是每个在句子中出现国家的概率
	 * */
	public void createRepeatWordResult(Map<String, Map<String, Double>> result,
			Map<String, List<String>> countryDic,
			Map<String, List<Integer>> commenWord) {
		for (Iterator<String> it = commenWord.keySet().iterator(); it.hasNext();) {
			String word = it.next();
	 ...
	 * 处理含有重复文字的结果
	 * */
	public Map<String, Map<String, Double>> dealResult(
			Map<String, Map<String, Double>> result) {
		Map<String, Map<String, Double>> finalResult = new TreeMap<String, Map<String, Double>>(
	 ...
	 *计算一句话关于提到各个国家的概率 
	 * */
	public void getSingleProbablity(Map<String, Double> result,
			Map<String, List<String[]>> countryWord,
			Map<String, List<Integer>> commenWord) {
		double totalProbality = 0;
		for (Iterator<String> it = countryWord.keySet().iterator(); it
				.hasNext();) {
			String key = it.next();
			...
			for (int i = 0; i < list1.size(); i++) {
				...
				for (Iterator<String> is = commenWord.keySet().iterator(); is
						.hasNext();) {
					String key1 = is.next();
	...
	public void getRepeatWordProbablity(
			Map<String, Map<String, Double>> result,
			Map<String, List<String[]>> countryWord,
			Map<String, List<Integer>> commenWord) {
		...
		for (Iterator<String> it = commenWord.keySet().iterator(); it.hasNext();) {
			...
			for (int j = 0; j < commList.size(); j++) {
				...
				double totalPro = 0;
				for (Iterator<String> ic = countryWord.keySet().iterator(); ic
						.hasNext();) {
				...
				for (Iterator<String> iw = wordCounPro.keySet().iterator(); iw
						.hasNext();) {
					String key = iw.next();
	 ...
	 * */
	public void getWordProbablity(Map<String, Map<String, Double>> result,
			Map<String, List<String[]>> countryWord,
			Map<String, List<Integer>> commenWord) {
		Map<String, Double> wordCounPro = new HashMap<String, Double>();// 存放一个单词对应每个国家的概率
		for (Iterator<String> it = commenWord.keySet().iterator(); it.hasNext();) {
	 ...
	 *  得到国家词和非国家词
	 * */
	public void getCouAndCommWord(Map<String, List<String>> countryDic,
			Map<String, List<String[]>> countryWord,
			Map<String, List<Integer>> commenWord, String str) throws Exception {
		// 对句子进行处理取出单词
		...
		for (int i = 0; i < words.size(); i++) {
			...
			// 先看是否为国家词
			for (Iterator<String> it = countryDic.keySet().iterator(); it
					.hasNext() && !(words.get(i)).matches("。+");) {
				...
				if (countryDic.get(key).contains(words.get(i))) {
					...
					if (countryWord.get(key) == null) {
						...
						tag1: for (int k = tempList.size() - 1; k >= 0; k--) {
	 ...
	 * 并得到每个句子属于各个国家的概率
	 * */
	public void getArticlePro(Map<String, Double> result,
			Map<String, List<String>> countryDic,
			Map<String, List<String[]>> countryWord,
			Map<String, List<Integer>> commenWord, String str) throws Exception {
	...
	public void getAllArticleProbability(int n) throws Exception {
		...
		// key存放国家 value为关于这个国家所在的单词列表，列表为长度为3的数组 1 国家单词 2 单词在句子的下标 3 非国家单词
		// 属于这个单词的概率
		...
		Map<String, List<String>> countryDic = test
				.getAllInformation("D:\\世界杯\\国家词典");
		// 获得保存最终结果的Map
		...
		while ((row = in.readLine()) != null) {
			...
			if (Integer.valueOf(fileIndex) == i) {
				...
				getArticlePro(result, countryDic, countryWord, commenWord,
						article);
				out.write(Integer.toString(i));
				for (Iterator<String> it = result.keySet().iterator(); it
						.hasNext();) {
					out.write("\t");
					String key = it.next();
	...
	public void getAllWordProbability() throws Exception {
		BufferedReader in = new BufferedReader(new FileReader(new File(
				"D:/世界杯/worldcup3_sen_10_nopunc.txt")));
		BufferedWriter out = new BufferedWriter(new FileWriter(new File(
				"D:/世界杯/WordProbability.txt")));
		String row;
		// key存放国家 value为关于这个国家所在的单词列表，列表为长度为3的数组 1 国家单词 2 单词在句子的下标 3 非国家单词
		// 属于这个单词的概率
		Map<String, List<String[]>> countryWord = new HashMap<String, List<String[]>>();
		...
		while ((row = in.readLine()) != null) {
			...
			for (Iterator<String> it = result.keySet().iterator(); it.hasNext();) {
				...
				for (Iterator<String> im = map.keySet().iterator(); im
						.hasNext();) {
					out.write("\t");
					String coun = im.next();
					if (map.get(coun) == 0)
